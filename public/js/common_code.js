...
function initDataTableAjax(el, config) {
    return el.DataTable({
        ...
        searching: config.hasOwnProperty('searching') ? config.searching : true,
        ajax: config.ajax,
        responsive: true,
        language: {
    ...
    return el.dropzone({
        ...
        init: function () {
            ...
            myDropzone.accept = function (file, done) {
                ...
                var maxFilesizeBytes = 1024*d.maxSize;
                if(file.size > maxFilesizeBytes){
                    showAlert('warning', 'El tamaño de la imagen debe ser menor a: ' +
                    d.maxSize+
                    ' KB ');
                    return this.removeFile(file);
                }
